...
class policies:
    def create_dgs(self,config,random_string_value):
        try:
            pulumi_devopsdg = oci.identity.DynamicGroup("pulumi_devopsdg",
                                                        ...
                                                        name=f"{config.get('dg_name')}_{random_string_value.result}",
                                                        )
    ...
    def create_policies(self,config,pulumi_devopsdg):
        try:
            pulumi_devops_policies = oci.identity.Policy("pulumi_devops_policies",
                                                         ...
                                                         name=f"{config.get('policies_name')}_{random_string()}",
                                                         statements=[
                                                             ...
                                                             f"Allow dynamic-group {pulumi_devopsdg.name} to manage all-resources in compartment id {config.get('compartment_ocid')}'",
                                                         ],
                                                         )
